    ...
    <div className="flex flex-col gap-7 mb-10">
      ...
      {options.map((option, _) => (
        <div key={option.value} className="flex gap-x-4 justify-between items-center">
          <label
            key={option.value}
            className={`bg-[#FCFCFC] box-border relative w-full flex justify-center items-center md:cursor-pointer shadow-[5px_5px_8px_0_#B4CBE280] overflow-hidden transition-colors duration-50 ease-linear border-2 ${selectedOptions.includes(option.value) ? 'border-blue' : 'border-[#FFFFFF33]'
              }`}
          >
            <input
