...
import { emptyPersonalLoanValidationState, IPersonalLoanValidationState, personalLoanValidationState } from '../atoms/validationStates/PersonalLoanValidationState';
...
const PersonalLoanPage = () => {
    ...
    const deletePersonalLoanAsync = async (index: number) => {
    ...
    const setPropertyValidationState = (index: number, key: keyof IPersonalLoanValidationState, value: string) => {
    ...
    const handleNextClick = async () => {
        ...
        formState.forEach(async (property, index) => {
            await savePersonalLoanAsync(property, index);
        })
    ...
    const addPersonalLoan = () => {
        ...
        setValidationState((prevState) => [
            ...prevState,
            emptyPersonalLoanValidationState
        ])
    ...
    return (
        <div className='flex flex-col justify-start h-full space-y-3 w-xl m-auto'>
            ...
            <div>
                        ...
                        <div key={formState[index].id} className='flex w-full justify-between items-center space-x-1 h-fit'>
                            <div className='w-full h-full'>
                                <CustomAccordion key={index} expanded={shouldExpandAccordion(index)}
                                    ...
                                    label={`PERSONAL LOAN ${index + 1}`}
                                    subTitle={
                                        currentItem !== index && !shouldExpandAccordion(index) ? getSubTitle(index) : ""
                                    }
                                >
                                    <PersonalLoanForm index={index} />
                                </CustomAccordion>
                            </div>
                            {
                                !shouldExpandAccordion(index) && (
                                    <button onClick={() => deletePersonalLoanAsync(index)} className='p-2 h-full bg-will-green'>
                ...
                <AddButton onClick={addPersonalLoan} label={`PERSONAL LOAN ${formState.length + 1}`} />
            </div>
